...
try {
    ...
    foreach ($module in $modules) {
        VM-Write-Log "INFO" "Attempting to install Python3 module: $($module.name)"
        ...
        if ($LastExitCode -eq 0) {
            VM-Write-Log "INFO" "Installed Python3 module: $($module.name)"
        } else {
            VM-Write-Log "ERROR" "Failed to install Python3 module: $($module.name)"
            $success = $false
        }
    }

    if ($success -eq $false) {
        VM-Write-Log "ERROR" "Failed to install at least one Python3 module"
