...
def update_github_url(package):
    ...
    matches = re.findall(
        r"[\"'](?P<url>https://github.com/(?P<org>[^/]+)/(?P<project>[^/]+)/releases/download/v?[^/]*?(?P<version>\d+(?:\.\d+)+)/[^\"']+)[\"']",
        content,
    )
    # Match also URLs like https://github.com/joxeankoret/diaphora/archive/refs/tags/3.0.zip
    matches += re.findall(
        r"[\"'](?P<url>https://github.com/(?P<org>[^/]+)/(?P<project>[^/]+)/archive/refs/tags/v?[^/]*?(?P<version>\d+(?:\.\d+)+)\.zip)[\"']",
        content,
    )
